...
class HttpService
{
    ...
    public function gateway($request)
    {
        ...
        # 允许header中可代理过去的参数
        $GATE_HEADER_ARG = config('hhttp.GATE_HEADER_ARG');
...
//        }
        ...
        if(config('hhttp.GATE_MODE') == 'strict'){
...
//            ContextService::set('setGatewayInfo',$gateway_info);
    ...
    public function getGatewayArg($input): mixed
    {
        ...
        # 忽略传递的input
        foreach (config('hhttp.GATE_IGNORE_INPUT') as $item){
            if(empty($item)){
                continue;
            }
            if(isset($input[$item])){
        ...
        if(is_string($value)){
            ...
            if(!empty($arg)){
                if(strncmp($value,self::$input_domain,6)===0){             # 数据取值 值域在input
                    ...
                    $value = config(config('hhttp.GATE_MODE_DEFAULT_STRICT_CONFIG').'.'.$value_[1])??'';
                }
            }
