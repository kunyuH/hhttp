...
use hhttp\io\common\Request\HmLoginRequest;
...
class LoginController extends BaseController
{
    ...
    public function login(HmLoginRequest $request)
    {
        ...
        if($login_error_num >= Config::get('hhttp.HP_LOGIN_RETRY')){
            return $this->resError([],'登录失败次数过多，请稍后再试！');
        }

        # 验证账密
        if($name != Config::get('hhttp.HP_NAME') || $password != Config::get('hhttp.HP_PASSWORD')){
